...
const TopBookCard = ({ book }) => {
  ...
  return (
    <motion.div
      ...
      className="bg-base-100 shadow-sm rounded-2xl p-4 flex flex-col md:flex-row gap-5 hover:shadow-2xl transition-shadow duration-700 items-center relative"
    >
      <img
        src={cover_photo}
        alt={book_title}
      ...
      <div className="flex-1 space-y-2 ">
        ...
        <div className="text-sm flex flex-wrap gap-2">
          <span className="text-[14px] md:text-[10px] text-primary-content badge bg-primary/40">{book_category}</span>
          <span className="text-[14px] md:text-[10px] text-primary-content badge bg-secondary/10">{reading_status}</span>
          <span className="text-[14px] md:text-[10px] text-base-content/70 badge bg-accent/30 ">Pages: {total_page}</span>
        </div>
        ...
        <Link to={`/books/${book._id}`} className="btn absolute bottom-4 right-4 btn-sm btn-soft border-blue-300 rounded-2xl px-7 hover:text-white btn-info">View Details</Link>
    </motion.div>
  );
};
